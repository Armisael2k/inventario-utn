...
import edit from "@/hooks/area/edit";
import remove from "@/hooks/area/remove";
...
function DataTable({ rows, loading, onUpdate, onDelete, onLoadingChange }, ref){
  ...
  const handleOpenConfirmDelete = () => rowSelectionModel.length > 0 ? setConfirmDeleteOpen(true) : toast.error("Ningún área seleccionada");
  ...
  return (
    <>
      ...
      >
        <Typography>¿Estas seguro de eliminar {pluralize("área", "s", rowSelectionModel.length, true)}?</Typography>
      </Modal>
    </>
  )
}
